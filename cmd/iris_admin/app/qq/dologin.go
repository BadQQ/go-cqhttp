...
// EncryptPasswordEnterWeb 密码加密的页面
...
// DoEncryptKeyInput bytekey的输入处理
...
// SessionTokenWeb html session_token 选择页面
...
// DoSessionTokenSelect session选择
...
func (l *Dologin) DoQrlogin(ctx iris.Context) (types.Panel, error) {
	...
	if ctx.Method() == "POST" {
		...
		case "enterSMS":
			...
			res, _ = l.Cli.SubmitSMS(text)
		case "captcha":
			text := ctx.FormValue("captchatext")
			res, _ = l.Cli.SubmitCaptcha(text, l.Captcha.Sig)
		}
	...
	if l.Cli.Uin != 0 {
		...
		header = "请使用手机QQ扫描二维码 (qrcode.png) : "
	}
	if l.Qrcode == nil {
		l.fetchQrCode()
	}
	_, err = l.Cli.QueryQRCodeStatus(l.Qrcode.Sig)
	if err != nil {
		l.fetchQrCode()
	}
...
// DoLoginBackend 异步处理登录请求
func (l *Dologin) DoLoginBackend() {
	for str := range l.Conn {
		fmt.Println(str)
		switch str {
		case "loginsuccess":
...
// loginResponseProcessorBackend 启动的时候自动登录 回包处理
		...
		case client.OtherLoginError, client.UnknownLoginError, client.TooManySMSRequestError:
			...
			if strings.Contains(msg, "版本") {
				...
				l.ErrMsg = struct {
					...
					Msg  string
					Step int
				}{Code: 2006, Msg: "自动登录失败，" + msg, Step: 2}
				return errors.New("密码错误或账号被冻结")
			}
			if strings.Contains(msg, "冻结") {
				l.ErrMsg = struct {
